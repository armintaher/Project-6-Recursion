...
int minRec2(int x[], int n) {
	...
	int min_Left = minRec2(x, n/2);
	int min_Right= minRec2(x + n/2, n - n/2);
...
double sqrtRec(double x, double low_guess, double high_guess) {
	double sqr = low_guess + (high_guess - low_guess) / 2;
    if (((high_guess - low_guess) /low_guess)< 1e-15) return sqr;
    else if (sqr * sqr > x) return sqrtRec(x, low_guess, sqr);
    else return  sqrtRec(x, sqr, high_guess);
}
...
int strCompare2(char* str1, char* str2) {
	int char1 = whatLetter(str1[0]);
    int char2 = whatLetter(str2[0]);
    if ((str1[0] == '\0') && str2[0] == '\0') return 0;
    else if ((char1 != -1) && (char2 != -1) && (char1 < char2) || (str1[0] == '\0' && char2 != -1)) return -1;
    else if ((char1 != -1) && (char2 != -1) && (char1 > char2) || (str2[0] == '\0' && char1 != -1)) return 1;
    if ((str1[0] != '\0') && (str2[0] != '\0') && (char1 == char2)) return strCompare2(str1 + 1, str2 + 1);
    else if ((char1 == -1) && ((char2 != -1) || (str2[0] == '\0'))) return  strCompare2(str1 +1, str2);
    else return  strCompare2(str1, str2 + 1);
}
...
int solveMazeRec(int row, int col) {
	if ((maze[row][col] == 1) || (maze[row][col] == 2)) return false;
    maze[row][col] = 2;
    if (row == MATRIX_SIZE -1) return true;
    if ((maze[row][col] != maze[0][col]) && (solveMazeRec(row - 1, col))) return true;
    if (solveMazeRec(row +1, col)) return true;
    if (solveMazeRec(row, col - 1)) return true;
    if (solveMazeRec(row, col + 1)) return true;
...
Martian change(int cents) {
    ...
    if (cents >=dodek_val){
        Coins = change(cents-dodek_val, nick_val, dodek_val);
        Coins.dodeks++;
    }
    else if(cents >= nick_val){
        Coins = change(cents-nick_val, nick_val, dodek_val);
        Coins.nicks++;
    }
    else if(cents < nick_val){
        Coins = change(cents-1, nick_val, dodek_val);
        Coins.pennies ++;
        return Coins;
    }
    if ((cents % dodek_val == 0) && Coins.pennies + Coins.dodeks + Coins.nicks > (cents/dodek_val)){
        Coins.dodeks = cents/dodek_val;
        Coins.nicks=0;
        Coins.pennies=0;
        return Coins;
    }
    if ((cents %nick_val == 0) && Coins.pennies + Coins.dodeks + Coins.nicks > (cents/nick_val)) {
        Coins.nicks = cents/nick_val;
        Coins.pennies=0;
        Coins.dodeks=0;
        return Coins;
    }
    if ((cents-1)%nick_val==0 && Coins.pennies + Coins.dodeks + Coins.nicks > ((cents-1)/nick_val)){
        Coins.nicks = cents/nick_val;
        Coins.pennies=1;
        Coins.dodeks=0;
    }
